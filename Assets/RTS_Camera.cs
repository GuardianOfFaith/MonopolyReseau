...
namespace RTS_Cam
{
    ...
    public class RTS_Camera : MonoBehaviour
    {
        ...
        public float followingSpeed = 5f; //speed when fol  lowing a target
        ...
        public float StartX = 50f; //x limit of map
        public float StartZ = 50f; //z limit of map
        ...
        private Sequence rotateleft;
        private Sequence rotateright;
        ...
        private void Update()
        {
            ...
            {
                rotating = true;
                rotateleft = DOTween.Sequence();
                rotateleft.Append(m_Transform.DORotate(new Vector3(90, this.transform.eulerAngles.y+90, 0),1)).OnComplete(endrotate);
                rotateleft.Play();
            }
            if (Input.GetKeyDown(KeyCode.A)&& !rotating)
            {
                rotating = true;
                rotateright = DOTween.Sequence();
                rotateright.Append(m_Transform.DORotate(new Vector3(90, this.transform.transform.eulerAngles.y-90, 0),1)).OnComplete(endrotate);
                rotateright.Play();
        ...
        private void LimitPosition()
        {
            ...
            m_Transform.position = new Vector3(Mathf.Clamp(m_Transform.position.x, StartX, limitX),
                m_Transform.position.y,
                Mathf.Clamp(m_Transform.position.z, StartZ, limitZ));
        }
